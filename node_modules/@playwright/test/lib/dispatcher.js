...
class Dispatcher {
  ...
  async _runJob(worker, testGroup) {
    ...
    const onDone = params => {
      ...
      massSkipTestsFromRemaining(new Set(params.skipTestsDueToSetupFailure), []); // Handle unexpected worker exit.

      if (params.unexpectedExitError) massSkipTestsFromRemaining(new Set(remaining.map(test => test._id)), [params.unexpectedExitError], true
      /* onlyStartedTests */
      );
