...
class Runner {
  ...
  async runAllTests(options = {}) {
    var _this$_reporter$onEnd, _this$_reporter2;
  ...
  async _runFiles(list, filesByProject, testFileReFilters) {
    ...
    filterByFocusedLine(preprocessRoot, testFileReFilters); // 4. Complain about only.
    ...
    if (!list) filterOnly(preprocessRoot); // 6. Generate projects.
    ...
    for (const [project, files] of filesByProject) {
      ...
      const projectSuite = new _test.Suite(project.name);
    ...
    if (result.status !== 'passed') return result; // 14. Run tests.
  ...
  async _performGlobalSetup(config, rootSuite, result) {
    ...
    const tearDown = async () => {
      ...
      await this._runAndReportError(async () => {
        if (globalSetupResult && config.globalTeardown) await (await this._loader.loadGlobalHook(config.globalTeardown, 'globalTeardown'))(this._loader.fullConfig());
      }, result);
  ...
  for (const projectSuite of rootSuite.suites) {
    for (const test of projectSuite.allTests()) {
      ...
      for (let parent = test.parent; parent; parent = parent.parent) {
        insideSerial = insideSerial || parent._parallelMode === 'serial'; // Serial cancels out any enclosing parallel.
