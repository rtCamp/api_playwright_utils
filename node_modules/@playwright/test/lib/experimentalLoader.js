...
async function load(moduleUrl, context, defaultLoad) {
  ...
  if (context.format !== 'commonjs' && context.format !== 'module' && context.format !== undefined) return defaultLoad(moduleUrl, context, defaultLoad); // Bail for built-in modules.

  if (!moduleUrl.startsWith('file://')) return defaultLoad(moduleUrl, context, defaultLoad);
  if (!moduleUrl.startsWith('file://')) return defaultLoad(moduleUrl, context, defaultLoad);

  const filename = _url.default.fileURLToPath(moduleUrl); // Bail for node_modules.
  ...
  const source = (0, _transform.transformHook)(code, filename, moduleUrl); // Output format is always the same as input format, if it was unknown, we always report modules.
